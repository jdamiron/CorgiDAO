...
contract CORGIBondingCalculator is IBondingCalculator {
  ...
  function markdown(address _pair) external view returns (uint256) {
    ...
    uint256 reserve;
    if (IUniswapV2Pair(_pair).token0() == CORGI) {
      reserve = reserve1;
    } else {
      reserve = reserve0;
    }
    return
      reserve.mul(2 * (10**IERC20(CORGI).decimals())).div(getTotalValue(_pair));
  }
}
