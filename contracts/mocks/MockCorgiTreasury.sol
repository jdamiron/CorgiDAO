...
contract MockCorgiTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public immutable CORGI;
  ...
  address public sCORGI;
  uint256 public sCORGIQueue; // Delays change to sCORGI address
  ...
  constructor(
    ...
    // isLiquidityToken[ _CORGIDAI ] = true;
    // liquidityTokens.push( _CORGIDAI );

    blocksNeededForQueue = _blocksNeededForQueue;
  }

  /**
        @notice allow approved address to deposit an asset for CORGI
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint CORGI needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(CORGI).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn CORGI for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    ICORGIERC20(CORGI).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sCORGI).balanceOf(msg.sender); // Can only borrow against sCORGI held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with CORGI
        @param _amount uint
     */
  function repayDebtWithCORGI(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    ICORGIERC20(CORGI).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, CORGI, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(CORGI).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(CORGI).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns CORGI valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match CORGI decimals
      value_ = _amount.mul(10**IERC20(CORGI).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SCORGI) {
      // 9
      sCORGIQueue = block.number.add(blocksNeededForQueue);
    } else return false;
